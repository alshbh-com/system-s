...
-- ========================================
-- Recreate ALL missing functions and triggers
-- ========================================

-- 1) Auto-fill shipping_cost from governorate
...
BEGIN
  ...
  IF (NEW.shipping_cost IS NULL OR NEW.shipping_cost = 0) AND NEW.governorate_id IS NOT NULL THEN
    SELECT shipping_cost INTO gov_shipping_cost FROM public.governorates WHERE id = NEW.governorate_id;
...
-- 2) Handle agent assignment (creates 'owed' payment, sets agent_shipping_cost)
...
-- 3) Handle order status change (delivered creates payment, reversal deletes it)
...
BEGIN
  ...
  -- Reversal: delivered -> shipped (delete delivered payment)
  ...
  -- Reversal: delivered -> non-shipped/delivered (delete all, unassign agent)
  ...
  -- Reversal: shipped -> non-shipped/delivered (delete owed, unassign)
...
-- 4) Handle order amount modification
...
BEGIN
  IF NEW.delivery_agent_id IS NOT NULL AND OLD.delivery_agent_id IS NOT NULL
    ...
    IF old_order_amount != new_order_amount THEN
      accounting_date := DATE(COALESCE(OLD.assigned_at, NEW.assigned_at, OLD.created_at, now()) AT TIME ZONE 'Africa/Cairo');
      ...
      IF amount_difference > 0 THEN
        INSERT INTO public.agent_payments (delivery_agent_id, order_id, amount, payment_type, payment_date, notes)
        VALUES (NEW.delivery_agent_id, NEW.id, -amount_difference, 'modification', accounting_date,
                'تعديل طلب رقم ' || COALESCE(NEW.order_number::text, NEW.id::text) || ' - خصم ' || amount_difference::text || ' ج.م');
      ELSIF amount_difference < 0 THEN
        INSERT INTO public.agent_payments (delivery_agent_id, order_id, amount, payment_type, payment_date, notes)
        VALUES (NEW.delivery_agent_id, NEW.id, ABS(amount_difference), 'modification', accounting_date,
...
BEFORE UPDATE OF total_amount, shipping_cost, agent_shipping_cost
ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.handle_order_amount_modification();


-- 5) Handle return creation
...
FOR EACH ROW
EXECUTE FUNCTION public.handle_return_creation();


-- 6) Handle partial return
